import { X, Package, Truck, ShieldCheck, DollarSign } from 'lucide-react';
import { supplierData, supplierPrices } from '../data/mockData';
...
export function SupplierComparisonModal({ product, onClose }: SupplierComparisonModalProps) {
  ...
  const availableSuppliers = supplierPrices
    .filter(sp => sp.productId === product.id)
    .map(sp => {
      const supplier = supplierData.find(s => s.id === sp.supplierId);
      return {
        ...sp,
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/5">
                  {availableSuppliers.map((supplier, idx) => (
                    <tr key={supplier.supplierId} className="group hover:bg-white/5 transition-colors">
                      <td className="py-4">
                      ...
                      <td className="py-4 text-white font-mono">${supplier.price.toFixed(2)}</td>
