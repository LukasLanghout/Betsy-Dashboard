// Dit component laat zien welke producten bijna op zijn.
// We vergelijken het voorraadniveau met het bestelpunt.
// Als het lager is, laten we een waarschuwing zien.
...
export function LowStockAlerts({ inventory, onDrillDown }: LowStockAlertsProps) {
  // We filteren de voorraad om alleen de producten met lage voorraad te vinden
  ...
  return (
    <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 flex flex-col h-full">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h3 className="text-white font-semibold">Actie Vereist: Lage Voorraad</h3>
      </div>
      
      <div className="flex-1 overflow-y-auto pr-2 space-y-3">
        {/* Als er geen waarschuwingen zijn, laten we dat ook weten */}
        {alerts.length === 0 ? (
          <div className="text-gray-400 text-sm text-center py-8">Geen lage voorraad waarschuwingen.</div>
        ) : (
          alerts.map(item => (
            <div 
              key={item.id}
