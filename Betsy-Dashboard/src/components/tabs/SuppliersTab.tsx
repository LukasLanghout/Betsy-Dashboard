// Dit is het overzicht van al onze leveranciers.
// We laten zien hoe betrouwbaar ze zijn en hoe lang het duurt voordat ze leveren.
// Handig om te weten bij wie we het beste kunnen bestellen!
...
export function SuppliersTab({ suppliers }: any) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {suppliers.length === 0 ? (
        <div className="col-span-3 py-20 text-center text-gray-500">Geen leveranciers gevonden.</div>
      ) : (
          ...
          <div key={i} className="bg-[#141414] border border-white/5 rounded-2xl p-6">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-[10px] font-bold text-emerald-500 bg-emerald-500/10 px-2 py-1 rounded-md">
                {((s.reliability_score || 0.9) * 100).toFixed(0)}% Betrouwbaar
              </span>
            </div>
            <div className="space-y-2 mb-6">
              ...
              <div className="flex justify-between text-xs">
                <span className="text-gray-500">Gem. Levertijd</span>
                <span className="text-gray-300">{s.delivery_days || 0} dagen</span>
              </div>
            </div>
